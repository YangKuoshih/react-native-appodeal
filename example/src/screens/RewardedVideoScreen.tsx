...
export default function RewardedVideoScreen({
  ...
  const loadRewardedVideo = () => {
    console.log('🔄 [RewardedVideo] Load button pressed - attempting to load ad...');
    ...
    console.log('✅ [RewardedVideo] cache() called - waiting for LOADED or FAILED_TO_LOAD event...');
  };

  const showRewardedVideo = () => {
    console.log('🎬 [RewardedVideo] Show button pressed - attempting to show ad...');
    ...
    if (initializationTime) {
      ...
      if (timeSinceInit < requiredWait) {
        const remainingSeconds = Math.ceil((requiredWait - timeSinceInit) / 1000);
        console.log('⏳ [RewardedVideo] OFFICIAL REQUIREMENT: Wait 15 seconds after initialization');
        console.log(`   ⚠️  Only ${Math.floor(timeSinceInit / 1000)} seconds have passed since initialization`);
        console.log(`   ⏳ Please wait ${remainingSeconds} more seconds before showing ads`);
        console.log('   📖 Source: Appodeal official documentation');
        return;
      } else {
        console.log(`✅ [RewardedVideo] ${Math.floor(timeSinceInit / 1000)} seconds since initialization - requirement met`);
      }
    } else {
      console.log('⚠️  [RewardedVideo] Initialization time not recorded. Proceeding anyway...');
    }
    ...
    if (!canShow) {
      console.log('❌ [RewardedVideo] Ad cannot be shown. Check SDK initialization and ad readiness.');
    ...
    try {
      ...
      console.log('   📊 If SHOWN fires, Display Rate should increase in dashboard.');
    } catch (error: any) {
  ...
  React.useEffect(() => {
    ...
    const onFailedToLoad = () => {
      ...
      console.log('   Check: 1) SDK initialized? 2) Network connection? 3) Dashboard config?');
    };
    const onExpired = () => {
      console.log('⏰ [RewardedVideo] EXPIRED event received - ad expired, need to reload');
    };
    const onShown = () => {
      console.log('🎉 [RewardedVideo] SHOWN event received - Ad is displaying!');
      console.log('   ✅ This should increase Display Rate in dashboard.');
      console.log('   📊 Display Rate should go from 0% to >0% after this.');
      console.log('   ⚠️  Note: Server confirmation comes via AD_REVENUE event.');
      console.log('   📊 Check dashboard in 15-30 minutes to verify server recorded it.');
    };
    const onFailedToShow = () => {
      console.log('❌ [RewardedVideo] FAILED_TO_SHOW event received - CRITICAL!');
    ...
    const onReward = (event: any) => {
      console.log('🎁 [RewardedVideo] REWARD event received - user earned reward!', event);
    };
    const onClicked = () => {
      console.log('👆 [RewardedVideo] CLICKED event received - user clicked the ad');
    };
    ...
    const onRevenue = (revenue: any) => {
      console.log('💰 [RewardedVideo] AD_REVENUE event received - SERVER CONFIRMED IMPRESSION!');
      ...
      console.log('   📊 This should appear in your dashboard within 15-30 minutes.');
    };
    Appodeal.addEventListener(AppodealSdkEvents.AD_REVENUE, onRevenue);

    console.log('✅ [RewardedVideo] Event listeners registered');
    console.log('💡 [RewardedVideo] Make sure SDK is initialized (use "Initialize Appodeal" on home screen)');
